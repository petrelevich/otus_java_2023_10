...
public class HttpHelloWorldServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) {
        ...
        var response =
                new DefaultFullHttpResponse(msg.protocolVersion(), OK, Unpooled.copiedBuffer(data, CharsetUtil.UTF_8));
        response.headers()
    ...
    private String processRequest(FullHttpRequest msg) {
        ...
        if (handler == null) {
            throw new ServerException("unexpected method:" + method);
        }
    ...
    private String getHandler(FullHttpRequest msg) {
        logger.atInfo().setMessage("msg.uri():{}").addArgument(msg::uri).log();
